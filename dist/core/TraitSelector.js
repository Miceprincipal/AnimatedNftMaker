...
exports.TraitSelector = void 0;
const errors_1 = require("../types/errors");
const RarityCalculator_1 = require("./RarityCalculator");
...
class TraitSelector {
    constructor(config, layerProcessor) {
        this.usedCombinations = new Set();
        this.traitCache = new Map();
        this.config = config;
        this.layerProcessor = layerProcessor;
        this.rarityTracker = new RarityCalculator_1.RarityCalculator();
    }
        ...
        while (combinations.length < count && attempts < maxAttempts) {
            ...
            for (const result of batchResults) {
                ...
                if (result.status === 'fulfilled') {
                    ...
                    if (error instanceof errors_1.GeneratorError && error.recoverable) {
                        // Soft failure, continue
                        continue;
                    }
                    logger_1.default.error(`Fatal error during generation attempt ${attempts}:`, error);
                }
            }
        }
        if (combinations.length < count) {
            logger_1.default.error(`Could only generate ${combinations.length} unique combinations out of ${count} requested after ${attempts} attempts.`);
        }
        // Complete progress tracking
        logger_1.default.info(`Trait combination generation completed`, {
    ...
    async generateSingleCombination(id) {
        let selectedTraits = [];
        ...
        for (const traitType of layerOrder) {
            ...
            if (traits.length === 0) {
                throw new errors_1.GeneratorError(errors_1.ErrorType.PROCESSING_ERROR, `All traits filtered out for layer ${traitType}. Retrying combination.`, { recoverable: true });
            }
     ...
     */
    applyIncompatibilityRules(availableTraits, selectedTraits) {
     ...
     */
    async applyDependentTraits(selectedTraits) {
        ...
        for (const [triggerKey, dependentKey] of Object.entries(this.config.dependent_traits)) {
            ...
            if (triggerIsSelected) {
                ...
                if (!isAlreadySelected) {
                    ...
                    const dependentTrait = dependentTraits.find(t => t.name.split('#')[0] === dependentName);
                    if (dependentTrait) {
                        dependentTraits.push(dependentTrait);
                        logger_1.default.debug(`Dependent trait added: ${dependentType}:${dependentName}`);
                    }
                }
            }
        }
     ...
     */
    applyExclusiveGroupRules(traitType, availableTraits, selectedTraits) {
        if (!this.config.exclusive_groups) {
            return availableTraits;
        }
        // Find the group that contains this trait type
        const group = Object.entries(this.config.exclusive_groups).find(([_, traits]) => traits.includes(traitType))?.[1];
        if (!group) {
            return availableTraits;
        }
     ...
     */
    applyConditionalRarity(availableTraits, selectedTraits) {
        if (!this.config.conditional_rarity) {
            return availableTraits;
        }
        // Create a deep clone to safely modify weights for this selection pass
        const adjustedTraits = JSON.parse(JSON.stringify(availableTraits));
            ...
            if (conditionIsMet) {
                ...
                for (const [targetKey, newWeight] of Object.entries(adjustments)) {
                    ...
                    if (targetTrait) {
                        ...
                        logger_1.default.debug(`Conditional Rarity: ${targetKey} weight set to ${newWeight} due to selection of ${conditionKey}`);
                    }
                }
            }
        }
     ...
     */
    async applyForcedPairings(traitType, selectedTraits) {
            ...
            if (triggerIsSelected) {
                ...
                if (forcedType === traitType) {
                    ...
                    if (!isAlreadySelected) {
                        ...
                        const forcedTrait = availableTraits.find(t => t.name.split('#')[0] === forcedName);
                        if (forcedTrait) {
                            logger_1.default.debug(`Forced pairing applied for layer ${traitType}: ${forcedKey}`);
    ...
    weightedRandomSelect(traits) {
        if (traits.length === 0) {
            throw new errors_1.GeneratorError(errors_1.ErrorType.PROCESSING_ERROR, 'No traits available for selection');
        }
        if (traits.length === 1) {
            return traits[0];
        }
    ...
    createTraitCombination(id, traits) {
    ...
    generateMetadata(id, traits) {
        ...
        try {
            ...
            if (fee > 0 && (!Array.isArray(creators) || creators.length === 0)) {
                logger_1.default.warn('Solana royalties set but creators array is empty. Add at least one creator for royalties to be tracked.');
            }
        }
        catch (_) { }
...
exports.TraitSelector = TraitSelector;
//# sourceMappingURL=TraitSelector.js.map